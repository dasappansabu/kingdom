...
p[i] = (np.sin(np.pi*t[i]/Ts*(1-beta))+ 4*beta*t[i]/Ts*np.cos(np.pi*t[i]/

Ts*(1+beta)))/(np.pi*t[i]/Ts*(1-(4*beta*t[i]/Ts)**2))
...
# Add Gaussian noise to s to simulate transmission via
a baseband AWGN channel
...
# Demodulate v_hat to the nearest 4-PAM point to
obtain decoded stream
